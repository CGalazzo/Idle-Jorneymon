(() => {
  ...
  function render() {
    ...
    status(isLinked() ? "Save vinculado à conta" : "Conta conectada; verificando seus saves…", isLinked() ? "ok" : "warn");
  }
  ...
  function saveTimestamp(save, rowUpdatedAt = "") {
    const savedAt = Number(save?.lastSavedAt) || 0;
    const rowAt = Date.parse(rowUpdatedAt || "") || 0;
    return Math.max(savedAt, rowAt);
  }

  function formatSaveDate(save, rowUpdatedAt = "") {
    const timestamp = saveTimestamp(save, rowUpdatedAt);
  ...
  function compareSaveTimes(local, cloudRow) {
    const localTime = saveTimestamp(local);
    const cloudTime = saveTimestamp(cloudRow?.save_data, cloudRow?.updated_at);
    if (cloudTime > localTime + SYNC_TOLERANCE_MS) return "cloud";
    if (localTime > cloudTime + SYNC_TOLERANCE_MS) return "local";
    return "same";
  }

  function saveSummary(save, rowUpdatedAt = "") {
  ...
  async function pushCloudSave(showFeedback = false, options = {}) {
    ...
    if (!force) {
      ...
      if (cloud.data?.save_data && compareSaveTimes(snapshot, cloud.data) === "cloud") {
        if (!showFeedback) {
          status("Save da conta mais recente — carregue-o antes de continuar", "warn");
          return false;
        }

        const replace = window.confirm(
          `O save da conta foi atualizado mais recentemente:\n${saveSummary(cloud.data.save_data, cloud.data.updated_at)}\n\n` +
          `Save deste aparelho:\n${saveSummary(snapshot)}\n\n` +
          "Deseja mesmo substituir o save da conta pelo progresso deste aparelho?"
        );
        if (!replace) {
          status("Sincronização cancelada; o save da conta foi preservado", "warn");
          return false;
        }
      }
    }
  ...
  async function linkCurrentSave() {
    ...
    if (data?.save_data) {
      const replace = window.confirm(
        `Já existe um save nesta conta:\n${saveSummary(data.save_data, data.updated_at)}\n\n` +
        `Save deste aparelho:\n${saveSummary(snapshot)}\n\n` +
        "Deseja substituir o save da conta pelo progresso atual deste aparelho?"
      );
      if (!replace) return;
    }

    setLinked(true);
    await pushCloudSave(true, { force: true });
    render();
  }
  ...
  async function loadCloudSave() {
    ...
    if (local?.hasStarted) {
      const newest = compareSaveTimes(local, data);
      const heading = newest === "cloud"
        ? "O save da conta é o mais recente."
        : newest === "local"
          ? "Atenção: o save deste aparelho parece mais recente que o save da conta."
          : "Os dois saves foram atualizados em horários próximos.";
      const replace = window.confirm(
        `${heading}\n\n` +
        `Save deste aparelho:\n${saveSummary(local)}\n\n` +
        `Save da conta:\n${saveSummary(data.save_data, data.updated_at)}\n\n` +
        "Deseja carregar exatamente o save da conta neste aparelho?"
      );
      if (!replace) return;
    }
  ...
  async function reconcileSignedInDevice() {
    ...
    if (!cloud.data?.save_data) {
      if (local?.hasStarted) {
        status("A conta ainda não tem save; vincule o progresso deste aparelho", "warn");
      } else {
        status("Conta conectada; comece uma jornada para vinculá-la", "warn");
      }
      setPanelOpen(true);
      return;
    }
    ...
    if (!local?.hasStarted) {
      ...
      setLinked(true);
      status("Save da conta carregado automaticamente", "ok");
      window.location.reload();
      return;
    }

    const newest = compareSaveTimes(local, cloud.data);
    if (newest === "cloud") {
      status("Save da conta é mais recente — use “Carregar save da conta”", "warn");
      setPanelOpen(true);
      return;
    }
    ...
    if (newest === "local") {
      if (isLinked()) {
        status("Este aparelho possui o save mais recente; preparando sincronização", "ok");
        queueCloudSave();
      } else {
        status("Save deste aparelho é mais recente; vincule-o para enviar à conta", "warn");
        setPanelOpen(true);
      }
      return;
    }
    ...
    setLinked(true);
    status("Save já está sincronizado entre os dispositivos", "ok");
  }
  ...
  async function initialize() {
    ...
    if (currentUser) await reconcileSignedInDevice();
    ...
    client.auth.onAuthStateChange((_event, session) => {
      ...
      if (currentUser) {
        setPanelOpen(true);
        window.setTimeout(() => reconcileSignedInDevice(), 0);
      }
    });
  }
